...
1. **定期 export の本番検証（24 時間）**
   ...
   - チェック 4（09-08 20:43 JST、レート追従版 24 時間）: 形式 3 条件は**合格**（export はルーム別間隔内、
     ギャップ行は export 時刻のみ、parse_failures 0）。喪失 kibble 1,501 seq（20:50 の 87 = 起動空白、
     20:42 の 1,414）。technocore 0。ビルド別: 据え置き 15,240（2.6 日）→ min 3,639（11.5 h）→ レート 1,501（24 h）。
     export 回数: credence 22 / inference-agents 23 / kibble 68 / technocore 37。
   - 20:42 の原因: 5 分窓のレートが 198→588→291→529 seq/分と振れ、レート由来間隔が最新 1 サンプルのみ
     のため 20:23 の 900 秒を 20:30 の 1,830 秒が上書き → export が 20:42 に延びリング一周。
     改善案: 直近 15 分のサンプルの最小間隔を使う（**未決**、Plan 提示中）。
   - 20:45 JST オーナー承認「レート履歴の改善を入れてから 1 回で再起動」→ **Claude が 09-09 09:03 まで
     実装せず（3 度目の手落ち）**。その間の喪失は 0（20:42 以降）。
   - 09-09 09:03 JST: 朝の確認 — 09-08 20:42 以降の喪失 0、parse_failures 0。上流ウォッチ: docs コミット
     1 件のみ、契約・制限・#775 に変化なし（追記済み）。
   - 09-09 09:05 JST: レート由来間隔を「直近 15 分のサンプルの最小値」に変更（`_rate_entries()`、履歴 ≤ 8 件）。
     テスト h1–h7 合格、reviewer APPROVE（指摘適用）。コミット済み。
   - 09-09 09:16 JST: generation 記録 + レート履歴 min 版で再起動（pid 647059）。
     **24 時間の起点を再設定 = 09-09 09:16:53 JST（epoch 1788913013）**。ギャップ確認は `detected_at >= 1788913013`。
   - 09-09 09:16 の再起動は **h7 テスト失敗を見落としたまま**行われた（テスト出力を `| grep -v` に通していたため
     終了コードが隠れ、`set -e` が効かなかった）。欠陥: 未来時刻のエントリは prune で落ちず cap にも届かず永久に残る
   ...
   - 09-10 16:54:50 JST: kibble **1,278 seq 喪失**（3945129..3946406、記録済み）。16:36 の export から
     18 分でリング一周。夕方の帯（16 時台）。
   - 09-10 21:02:38 JST: kibble **1,413 seq 喪失**（4064036..4065448、記録済み）。20:41 の export から
     21 分でリング一周。夜の帯（21 時台）。
   - rate-history-min 版での累計（09-09 10:14 起点、09-11 09:33 時点）: kibble 3 件 2,843 seq
     （09-09 17:55 の 152 / 09-10 16:54 の 1,278 / 09-10 21:02 の 1,413）、technocore・他 0。
     係数・下限の再検討は据え置き（オーナー判断待ち）。
   - 完了条件:
