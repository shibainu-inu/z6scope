...
- **テスト実行はパイプで終了コードを隠さない**（`set -o pipefail`、CLAUDE.md に明記）— 09-09 09:16、h7 の
  失敗が `| grep -v` で隠れたまま commit `b729041` を再起動してしまった。欠陥は「未来時刻のレートエントリが
  永久に残る」（クロック後退時のみ）。修正: 未来時刻のエントリを prune と読み出しの両方で即時破棄。
